...
export const reviewService = {
  async getQueue(status: ReviewStatus, limit: number) {
    ...
    return parseSuccessResponse(api.review.queue.responses[200], response.data);
  },
  async getHistory(wordId: number) {
    const response = await apiClient.get(buildApiUrl(`/api/review/words/${wordId}/history`));
    return parseSuccessResponse(api.review.history.responses[200], response.data);
  },
  async transition(payload: { id: number; toStatus: ReviewStatus; notes?: string }) {
    ...
    return parseSuccessResponse(api.review.transition.responses[200], response.data);
  },
  async bulkTransition(payload: { ids: number[]; toStatus: ReviewStatus; notes?: string }) {
    ...
    return parseSuccessResponse(api.review.bulkTransition.responses[200], response.data);
  },
