...
describe("ChartStyle", () => {
  it("renders theme selectors for the implemented named themes", () => {
    ...
    expect(style?.innerHTML.includes(".soothing [data-chart=chart-1]")).toBe(true);
    ...
    expect(style?.innerHTML.includes("--color-score: #1f5f63;")).toBe(true);
  ...
  it("falls back to a color entry when a themed value is not provided", () => {
    ...
    expect(style?.innerHTML.includes(".soothing [data-chart=chart-2]")).toBe(true);
    ...
    expect(style?.innerHTML.match(/--color-attempts: #123456;/g)?.length).toBe(7);
  });
});
