...
- Additional implementation completed after the initial rollout:
  ...
  - full `pnpm run lint`, `pnpm run test:ui`, `pnpm test`, and `pnpm run test:e2e:smoke:development` now pass on the current branch state, with the UI suite now at 35 files / 67 tests and the backend suite now at 59 tests with 57 passing and 2 DB-dependent skips
