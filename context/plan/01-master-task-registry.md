...
| P8-005 | doing | Expand high-confidence seed content (new words + linked sentences) using current seed correctness rules | P8-002 | M | `words.json` and `sentences.json` both grow with unique entries; each new word has >=2 linked sentences; validator + single-source checks pass |
