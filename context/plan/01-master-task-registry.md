...
| ID | Status | Task | Depends On | Effort | Done When |
|---|---|---|---|---|---|
| P7-001 | done | Add explicit `.env` runtime controls for hybrid quiz audio behavior across local and production parity | - | S | `.env` includes required audio/runtime keys, server/client read them safely, and app has deterministic behavior when values are missing |
...
| P8-006 | hold | Production deploy + CI/CD pipeline (build/test/deploy) with environment strategy and rollback path | P8-002,P8-003,P8-004,P8-005,P8-007,P8-008 | L | app is deployed to a stable URL, pipeline runs lint/test/build on push, deploys on main, and docs include env setup + rollback steps |
