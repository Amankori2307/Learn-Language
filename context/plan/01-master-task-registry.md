...
|---|---|---|---|---|---|
| P6-001 | todo | Build language-isolation endpoint matrix and implementation checklist for all learner-facing APIs (`quiz`, `stats`, `attempts`, `clusters`, `leaderboard`, `words`) | - | S | checklist maps every endpoint to expected `(userId, language)` behavior with test ownership |
...
| P6-006 | todo | CI/compose parity check for lint/test/content pipeline and deterministic DB bootstrap | P6-004,P6-005 | S | same command set passes in local + docker + GitHub Actions without manual overrides |
